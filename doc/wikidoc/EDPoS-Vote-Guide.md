## 1. ready For DPOS vote
**NOTE:**  

1) MetabaseNet Core use the PoW + DPoS consensus model, and there is no pre-excavation and pre-projection, it is necessary to dig out enough Token through the PoW before Token can be used to vote
...
# Dpos node reward ratio (range: 0~10000)
rewardratio=500
...
**NOTE:** DPoS voting consists of voting by oneself and voting by other address

### 6.1 Voting by the DPoS template address owner
```
command:
metabasenet sendfrom <DPoS spending address>  <DPOS template address>  <$TOKEN>
