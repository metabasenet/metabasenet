...
BOOST_AUTO_TEST_CASE(uint256id)
{
    ...
    uint256 nid = uint256(uint16(0x1122), uint32(0xaabbccdd), uint256(0x123456789abc));
    for (int i = 0; i < 8; i++)
    {
    ...
    uint256 tid1 = uint256(uint16(1), uint32(0xa), tid);
    uint256 tid2 = uint256(uint16(2), uint32(0xb), tid);
    uint256 tid3 = uint256(uint16(3), uint32(0xc), tid);
