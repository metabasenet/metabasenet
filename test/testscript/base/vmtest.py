...
def getreverseaddresskey(address):
    return reversehex(getaddresskey(address))
...
def createContract(fromaddress, file, fork, cparam):
    ...
    txid,err = createcontract(fromaddress, '0', 0, fork, wasmcode, cparam)
...
def muxcontracttest(fromaddress, fork, cparam, muxtype, name, codefile, typein, describe, sourcefile):
    ...
    unlockkey(fromaddress)
    txid,err = createmuxcontract(fromaddress, '0', 0, fork, muxcode, cparam)
