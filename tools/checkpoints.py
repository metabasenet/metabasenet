...
def get_block_hash(height):
    json_str = str(subprocess.check_output(
        ['metabasenet', 'getblockhash', str(height)]))
...
def get_block(block_hash):
    json_str = subprocess.check_output(['metabasenet', 'getblock', block_hash])
    return json.loads(json_str)


def get_primary_fork_height():
    return int(subprocess.check_output(['metabasenet', 'getforkheight']))
