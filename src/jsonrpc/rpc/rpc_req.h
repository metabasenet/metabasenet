...
public:
    ...
    void SetParamJson(const std::string& str)
    {
        strParamJson = str;
    }
    const std::string& GetParamJson() const
    {
        return strParamJson;
    }

protected:
    std::string strParamJson;
};

typedef std::shared_ptr<CRPCParam> CRPCParamPtr;

class CRPCCommonParam : public CRPCParam
{
public:
...
public:
    CRPCCommonParam(const std::string& strMethodIn)
      : strMethod(strMethodIn) {}
    ~CRPCCommonParam() = default;
    ...
    CRPCCommonParam& FromJSON(const json_spirit::Value& v)
    {
        val = v;
        return *this;
    }
...
CRPCReqVec DeserializeCRPCReq(const std::string& str, const std::set<std::string>& setNoParserMethod, bool& fArray);
