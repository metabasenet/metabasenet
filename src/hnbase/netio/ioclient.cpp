                                   ...
                                   boost::asio::ssl::verify_context& ctx)
{
    ...
    if (fPreverified)
    {
        hnbase::StdDebug("SSLVERIFY", (string("SSL verify success, subject: ") + subject_name).c_str());
    }
    else
    {
        string sErrorDesc;
        ...
        hnbase::StdDebug("SSLVERIFY", (string("SSL verify fail, subject: ") + subject_name + string(", cts_error: [") + to_string(cts_error) + string("]<") + to_string(depth) + string(">  ") + string(sErrorDesc)).c_str());
    }
