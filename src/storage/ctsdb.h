...
class CCTSIndex : public mtbase::CKVDB
{
public:
    CCTSIndex();
    ~CCTSIndex();
...
protected:
    void Serialize(mtbase::CStream& s, mtbase::SaveType& opt)
    {
        mtbase::CVarInt var(basetype::size());
        s.Serialize(var, opt);
        s.Write((const char*)&((*this)[0]), basetype::size() * sizeof(std::pair<K, V>));
    }
    void Serialize(mtbase::CStream& s, mtbase::LoadType& opt)
    {
        mtbase::CVarInt var;
        s.Serialize(var, opt);
    ...
    {
        mtbase::CVarInt var(basetype::size());
        serSize += mtbase::GetSerializeSize(var) + basetype::size() * sizeof(std::pair<K, V>);
    }
};
...
protected:
    ...
    void Serialize(mtbase::CStream& s, std::size_t& serSize)
    {
        ...
        mtbase::CVarInt var(strSnappy.size());
        serSize += mtbase::GetSerializeSize(var) + strSnappy.size();
    }
};
...
public:
    ...
    {
        mtbase::CUpgradeLock ulock(rwMap);
