...
bool CTrieDB::AddNewTrie(const uint256& hashPrevRoot, const bytesmap& mapKvList, uint256& hashNewRoot)
{
    ...
    {
        StdLog("CTrieDB", "AddNewTrie: Create trie node list fail, prev root: %s", hashPrevRoot.GetHex().c_str());
        return false;
    }
    ...
    for (auto& kv : mapCacheNode)
    {
        ...
        {
            StdLog("CTrieDB", "AddNewTrie: Write fail, prev root: %s", hashPrevRoot.GetHex().c_str());
            return false;
        }
    }
...
bool CTrieDB::SaveCacheTrie(std::map<uint256, CTrieValue>& mapCacheNode)
{
    ...
    for (auto& kv : mapCacheNode)
    {
        ...
        if (!Write(ssKey, ssValue))
        {
            StdLog("CTrieDB", "SaveCacheTrie: Write fail");
            return false;
        }
    }
    return true;
...
bool CTrieDB::SetValueNode(const uint256& hashValueNode, const bytes& btValue)
{
    ...
    CTrieValue value;
    value.type = CTrieValue::TYPE_VALUE;
    value.vaValue = btValue;

    value.GetStream(ssValue);

    if (!Write(ssKey, ssValue))
    {
        StdLog("CTrieDB", "SetValueNode: Write fail");
        return false;
    }
    return true;
}
...
bool CTrieDB::CreateTrieNodeList(const uint256& hashPrevRoot, const bytesmap& mapKvList, uint256& hashNewRoot, std::map<uint256, CTrieValue>& mapCacheNode)
{
    ...
    for (const auto& kv : mapKvList)
    {
        ...
        {
            StdLog("CTrieDB", "CreateTrieNodeList: Add node fail, prev root: %s", hashPrevRoot.GetHex().c_str());
            return false;
        }
    }
...
bool CTrieDB::AddNode(uint256& hashRoot, const bytes& nbKeyNibble, const bytes& btValue, std::map<uint256, CTrieValue>& mapCacheNode)
{
    ...
    {
        StdLog("CTrieDB", "AddNode: Get node path fail, root hash: %s", hashRoot.GetHex().c_str());
        return false;
    }
    ...
    for (int64 i = path.size() - 2; i >= 0; i--)
    {
        ...
        default:
            StdLog("CTrieDB", "AddNode: Old node type error, type: %d, hash: %s", kv.value.type, kv.hashValue.GetHex().c_str());
            return false;
        }
    }
    ...
    for (int64 i = path.size() - 2; i >= 0; i--)
    {
                ...
                if (kv.keyNew.size() > 0)
                {
                    if (fIsValue)
                    {
                        if (hashPrev != 0 && kv.value.vaBranch.GetValueHash(kv.keyNew[0]) == hashPrev)
                        {
                            ...
                            printf("AddNode: Branch value same, value: %s, vaue hash: %s\n", strValue.c_str(), hashPrev.GetHex().c_str());
#endif
                            return true;
                        }
...
#ifdef TEST_FLAG
                        std::string strValue(btValue.begin(), btValue.end());
                        printf("AddNode: Extension value same, value: %s, vaue hash: %s\n", strValue.c_str(), hashPrev.GetHex().c_str());
#endif
                        return true;
                    }
        ...
        case CTrieValue::TYPE_VALUE:
        default:
            StdLog("CTrieDB", "AddNode: New node type error, type: %d, hash: %s", kv.value.type, kv.hashValue.GetHex().c_str());
            return false;
        }
    }
    hashRoot = hashPrev;
...
bool CTrieDB::GetDbNodeValue(const uint256& hash, CTrieValue& value)
{
    ...
    {
        StdLog("CTrieDB", "GetDbNodeValue: Read fail, hash: %s", hash.GetHex().c_str());
        return false;
    }
    if (!value.SetStream(ssValue))
    {
        StdLog("CTrieDB", "GetDbNodeValue: Set stream fail, hash: %s", hash.GetHex().c_str());
        return false;
    }
    return true;
}
...
bool CTrieDB::GetNodePath(const uint256& hashRoot, const bytes& nbKeyNibble, TRIE_NODE_PATH& path, std::vector<uint256>& vRemove, std::map<uint256, CTrieValue>& mapCacheNode)
{
    ...
    while (!syKey.empty())
    {
        ...
        if (!GetNodeValue(hash, value, fCache, mapCacheNode))
        {
            ...
            StdLog("CTrieDB", "GetNodePath: Get node fail, hash: %s", hash.GetHex().c_str());
            return false;
        }
        ...
        case CTrieValue::TYPE_VALUE:
        default:
            StdLog("CTrieDB", "GetNodePath: node type error, type: %d, hash: %s", value.type, hash.GetHex().c_str());
            return false;
        }
    }
    return true;
...
bool CTrieDB::GetKeyValue(const uint256& hashRoot, const bytes& nbKey, bytes& btValue)
{
    ...
    while (!syKey.empty())
    {
        ...
        if (!GetDbNodeValue(hash, value))
        {
            StdLog("CTrieDB", "GetKeyValue: Get db node fail, hash: %s", hash.GetHex().c_str());
            return false;
        }
        ...
        case CTrieValue::TYPE_VALUE:
        default:
            StdLog("CTrieDB", "GetKeyValue: node type error, type: %d", value.type);
            return false;
        }
    ...
    if (!GetDbNodeValue(hash, value))
    {
        StdLog("CTrieDB", "GetKeyValue: Get db value fail, hash: %s", hash.GetHex().c_str());
        return false;
    }
    if (value.type != CTrieValue::TYPE_VALUE)
    {
        StdLog("CTrieDB", "GetKeyValue: Db value type error, type: %d, hash: %s", value.type, hash.GetHex().c_str());
        return false;
    }
    btValue = value.vaValue;
    return true;
