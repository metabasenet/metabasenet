...
public:
    BlockHeader() {}
    BlockHeader(const int64_t m_number_in, const u256& m_gasLimit_in,
            const int64_t m_timestamp_in, const Address& m_author_in, const u256& m_difficulty_in) 
        : m_number(m_number_in), m_gasLimit(m_gasLimit_in), m_timestamp(m_timestamp_in),
        m_author(m_author_in), m_difficulty(m_difficulty_in) {}

    void setTimestamp(int64_t _v) { m_timestamp = _v; }
    void setAuthor(Address const& _v) { m_author = _v; }
    void setNumber(int64_t _v) { m_number = _v; }
