...
public:
    ...
    TransactionBase(uint64_t const _chainId, u256 const& _value, u256 const& _gasPrice, u256 const& _gas, Address const& _dest, bytes const& _data, u256 const& _nonce, Secret const& _secret): m_type(MessageCall), m_nonce(_nonce), m_value(_value), m_receiveAddress(_dest), m_gasPrice(_gasPrice), m_gas(_gas), m_data(_data) { if (_chainId != 0) setChainId(_chainId); sign(_secret); }

    /// Constructs a signed contract-creation transaction.
    TransactionBase(uint64_t const _chainId, u256 const& _value, u256 const& _gasPrice, u256 const& _gas, bytes const& _data, u256 const& _nonce, Secret const& _secret): m_type(ContractCreation), m_nonce(_nonce), m_value(_value), m_gasPrice(_gasPrice), m_gas(_gas), m_data(_data) { if (_chainId != 0) setChainId(_chainId); sign(_secret); }
    ...
    bool hasSignature() const { return !!m_vrs; }
    ...
    bool isReplayProtected() const { return !!m_chainId; }

    uint64_t getChainId() const { return (m_chainId ? *m_chainId : 0); }
    void setChainId(const uint64_t nChainId) { m_chainId = nChainId; }
    ...
    void setSignature(const bytes& sigData);
