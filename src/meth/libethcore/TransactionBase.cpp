    ...
    try
    {
        ...
        else
        {
            ...
            auto const recoveryID =
                m_chainId ? byte{v - (u256{*m_chainId} * 2 + 35)} : byte{v - 27};
...
bytes TransactionBase::getSignature() const
{
    if (!m_vrs)
        return bytes();

    return ((Signature)(*m_vrs)).asBytes();
}

void TransactionBase::setSignature(const bytes& sigData)
{
    m_vrs = SignatureStruct(Signature(sigData));
}
...
u256 TransactionBase::rawV() const
{
    ...
    int const vOffset = m_chainId ? *m_chainId * 2 + 35 : 27;
...
{
    if (m_chainId && *m_chainId != _chainId)
        BOOST_THROW_EXCEPTION(InvalidSignature());
}
