...
extern "C" evmc_vm* evmc_create_aleth_interpreter() noexcept
{
    ...
    static evmc_vm s_vm{
        EVMC_ABI_VERSION, "interpreter", "1.8.0-8+commit.5d1078ac.dirty"/*aleth_version*/, ::destroy, ::execute, getCapabilities,
        nullptr,  // set_option
    };
...
void VM::interpretCases()
{
    ...
    DO_CASES
    {
        ...
        CASE(EXP)
        {
            ...
            m_runGas = toInt63(VMSchedule::stepGas5 + byteCost * intx::count_significant_words<256>(expon));
            ON_OP();
            updateIOGas();
        ...
        CASE(PREVRANDAO)
        {
            ...
            m_SPP[0] = intx::be::load<intx::uint256>(getTxContext().block_prev_randao);
        }
        NEXT
