...
#define GET_HASH_HEIGHT(BLOCKHASH) BLOCKHASH.GetB2()
                       ...
                       CDelegateEvolveResult& result, const uint256& hashBlock)
{
    ...
    // init
    {
        ...
        if (it != mapVote.end())
        {
            StdLog("CDelegate", "Evolve Setup: already exist, target height: %d, setup block: [%d] %s",
                   nTarget, GET_HASH_HEIGHT(hashBlock), hashBlock.GetHex().c_str());
        }
        else
        {
            ...
            StdDebug("CDelegate", "Evolve Setup: target height: %d, time: %ld us, setup block: [%d] %s",
                     nTarget, (t1 - t0).ticks(), GET_HASH_HEIGHT(hashBlock), hashBlock.GetHex().c_str());
        }
    }
    ...
    // enroll & distribute
    {
        ...
        do
        {
            if (it == mapVote.end())
            {
                StdLog("CDelegate", "Evolve Enroll: find target height fail, target height: %d", nEnrollEnd);
                break;
            }
            ...
            StdDebug("CDelegate", "Evolve Enroll: target height: %d, time: %ld us, distribute block: [%d] %s",
                     nEnrollEnd, (t1 - t0).ticks(), GET_HASH_HEIGHT(hashBlock), hashBlock.GetHex().c_str());
        } while (0);
    }
    ...
    // publish
    {
        ...
        do
        {
            if (it == mapVote.end())
            {
                StdLog("CDelegate", "Evolve Publish: find target height fail, target height: %d", nPublish);
                break;
            }

            if (it->second.hashDistributeBlock == 0)
            {
                StdLog("CDelegate", "Evolve Publish: hashDistributeBlock is null, target height: %d", nPublish);
                break;
            }
            ...
            {
                StdLog("CDelegate", "Evolve Publish: find distribute vote fail, target height: %d, distribute block: [%d] %s",
                       nPublish, GET_HASH_HEIGHT(hashDistribute), hashDistribute.GetHex().c_str());
                break;
            }
            ...
            if (!vote.is_enroll)
            {
                StdLog("CDelegate", "Evolve Publish: not is enroll, target height: %d, distribute block: [%d] %s",
                       nPublish, GET_HASH_HEIGHT(hashDistribute), hashDistribute.GetHex().c_str());
                break;
            }
            ...
            {
                StdLog("CDelegate", "Evolve Publish: already public, target height: %d, distribute block: [%d] %s",
                       nPublish, GET_HASH_HEIGHT(hashDistribute), hashDistribute.GetHex().c_str());
            ...
            StdDebug("CDelegate", "Evolve Publish: target height: %d, time: %ld us, distribute block: [%d] %s",
                     nPublish, (t1 - t0).ticks(), GET_HASH_HEIGHT(hashDistribute), hashDistribute.GetHex().c_str());
        } while (0);
    }
}
...
void CDelegate::GetEvolveData(int nBlockHeight, CDelegateEvolveResult& result, const uint256& hashBlock)
{
    ...
    // distribute
    {
        ...
        do
        {
            if (it == mapVote.end())
            {
                StdLog("CDelegate", "GetEvolveData Enroll: find target height fail, target height: %d", nEnrollEnd);
                break;
            }
            if (it->second.hashDistributeBlock == 0)
            {
                StdLog("CDelegate", "GetEvolveData Enroll: hashDistributeBlock is 0, target height: %d", nEnrollEnd);
                break;
            }
            if (it->second.hashDistributeBlock != hashBlock)
            {
                StdLog("CDelegate", "GetEvolveData Enroll: hashDistributeBlock not is hashBlock, target height: %d, hashDistributeBlock: %s, hashBlock: %s",
                       nEnrollEnd, it->second.hashDistributeBlock.GetHex().c_str(), hashBlock.GetHex().c_str());
                break;
            }
            ...
            if (mt == mapDistributeVote.end())
            {
                StdLog("CDelegate", "GetEvolveData Enroll: find distribute vote fail, target height: %d, distribute block: [%d] %s",
                       nEnrollEnd, GET_HASH_HEIGHT(it->second.hashDistributeBlock), it->second.hashDistributeBlock.GetHex().c_str());
                break;
            }
    ...
    // publish
    {
        ...
        do
        {
            if (it == mapVote.end())
            {
                StdLog("CDelegate", "GetEvolveData Publish: find target height fail, target height: %d", nPublish);
                break;
            }
            if (it->second.hashDistributeBlock == 0)
            {
                StdLog("CDelegate", "GetEvolveData Publish: hashDistributeBlock is null, target height: %d", nPublish);
                break;
            }
            map<uint256, CDelegateVote>::iterator mt = mapDistributeVote.find(it->second.hashDistributeBlock);
            if (mt == mapDistributeVote.end())
            {
                StdLog("CDelegate", "GetEvolveData Publish: find distribute vote fail, target height: %d, distribute block: [%d] %s",
                       nPublish, GET_HASH_HEIGHT(it->second.hashDistributeBlock), it->second.hashDistributeBlock.GetHex().c_str());
                break;
            }
                                 ...
                                 const vector<unsigned char>& vchDistributeData)
{
    ...
    if (it == mapVote.end())
    {
        StdLog("CDelegate", "HandleDistribute: find target height fail, target height: %d", nTargetHeight);
        return false;
    }
    const uint256& hashDistribute = it->second.hashDistributeBlock;
    if (hashDistribute == 0)
    {
        StdLog("CDelegate", "HandleDistribute: hashDistributeBlock is null, target height: %d", nTargetHeight);
        return false;
    }
    if (hashDistribute != hashDistributeAnchor)
    {
        StdLog("CDelegate", "HandleDistribute: distribute anchor error, target height: %d, local distribute: %s, anchor distribute: %s",
    ...
    if (mt == mapDistributeVote.end())
    {
        StdLog("CDelegate", "HandleDistribute: find distribute vote fail, target height: %d", nTargetHeight);
        return false;
    }
    CDelegateVote& vote = mt->second;
    ...
    StdDebug("CDelegate", "HandleDistribute: Accept target height: %d, time: %ld us, ret: %s, distribute block: [%d] %s",
             ...
             GET_HASH_HEIGHT(hashDistribute), hashDistribute.GetHex().c_str());
    return ret;
}
                              ...
                              const vector<unsigned char>& vchPublishData, bool& fCompleted)
{
    ...
    if (it == mapVote.end())
    {
        StdLog("CDelegate", "HandlePublish: find target height fail, height: %d", nTargetHeight);
        return false;
    }
    const uint256& hashDistribute = it->second.hashDistributeBlock;
    if (hashDistribute == 0)
    {
        StdLog("CDelegate", "HandlePublish: hashDistributeBlock is null, target height: %d", nTargetHeight);
        return false;
    }
    if (hashDistribute != hashDistributeAnchor)
    {
        StdLog("CDelegate", "HandlePublish: distribute anchor error, target height: %d, local distribute: %s, anchor distribute: %s",
               nTargetHeight, hashDistribute.GetHex().c_str(), hashDistributeAnchor.GetHex().c_str());
        return false;
    }
    map<uint256, CDelegateVote>::iterator mt = mapDistributeVote.find(hashDistribute);
    if (mt == mapDistributeVote.end())
    {
        StdLog("CDelegate", "HandlePublish: find distribute vote fail, target height: %d", nTargetHeight);
        return false;
    }
    CDelegateVote& vote = mt->second;
...
void CDelegate::GetAgreement(int nTargetHeight, const uint256& hashDistributeAnchor, uint256& nAgreement, size_t& nWeight, map<CDestination, size_t>& mapBallot)
{
    ...
    if (it == mapVote.end())
    {
        StdLog("CDelegate", "Get agreement: find target height fail, height: %d", nTargetHeight);
        return;
    }
    const uint256& hashDistribute = it->second.hashDistributeBlock;
    if (hashDistribute == 0)
    {
        StdLog("CDelegate", "Get agreement: hashDistributeBlock is null, target height: %d", nTargetHeight);
        return;
    }
    if (hashDistribute != hashDistributeAnchor)
    {
        StdLog("CDelegate", "Get agreement: hashDistributeBlock error, target height: %d, local distribute: %s, anchor distribute: %s",
               nTargetHeight, hashDistribute.GetHex().c_str(), hashDistributeAnchor.GetHex().c_str());
        return;
    }
    map<uint256, CDelegateVote>::iterator mt = mapDistributeVote.find(hashDistribute);
    if (mt == mapDistributeVote.end())
    {
        StdLog("CDelegate", "Get agreement: find distribute vote fail, target height: %d", nTargetHeight);
        return;
    }
...
void CDelegate::GetProof(int nTargetHeight, vector<unsigned char>& vchDelegateData)
{
    map<int, CDelegateVote>::iterator it = mapVote.find(nTargetHeight);
    if (it == mapVote.end())
    {
        StdLog("CDelegate", "Get proof: find target height fail, height: %d", nTargetHeight);
        return;
    }
    const uint256& hashDistribute = it->second.hashDistributeBlock;
    if (hashDistribute == 0)
...
bool CDelegate::IsCompleted(int nTargetHeight)
{
    ...
    if (it == mapVote.end())
    {
        StdLog("CDelegate", "Is completed: find target height fail, height: %d", nTargetHeight);
        return false;
    }
    const uint256& hashDistribute = it->second.hashDistributeBlock;
    if (hashDistribute == 0)
    {
        StdLog("CDelegate", "Is completed: hashDistributeBlock is null, target height: %d", nTargetHeight);
        return false;
    }
    map<uint256, CDelegateVote>::iterator mt = mapDistributeVote.find(hashDistribute);
    if (mt == mapDistributeVote.end())
    {
        StdLog("CDelegate", "Is completed: find distribute vote fail, target height: %d", nTargetHeight);
        return false;
    }
...
int64 CDelegate::GetPublishedTime(int nTargetHeight)
{
    ...
    if (it == mapVote.end())
    {
        StdLog("CDelegate", "Get publish time: find target height fail, target height: %d", nTargetHeight);
        return -1;
    }
    const uint256& hashDistribute = it->second.hashDistributeBlock;
    if (hashDistribute == 0)
    {
        StdLog("CDelegate", "Get publish time: hashDistributeBlock is null, target height: %d", nTargetHeight);
        return -1;
    }
    map<uint256, CDelegateVote>::iterator mt = mapDistributeVote.find(hashDistribute);
    if (mt == mapDistributeVote.end())
    {
        StdLog("CDelegate", "Get publish time: find distribute vote fail, target height: %d", nTargetHeight);
        return -1;
    }
    CDelegateVote& vote = mt->second;
