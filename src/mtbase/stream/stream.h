...
protected:
    ...
    CStream& Serialize(const std::set<K, C, A>& t, ObjectType&, SaveType&);
    template <typename K, typename C, typename A>
    CStream& Serialize(std::set<K, C, A>& t, ObjectType&, SaveType&);
    template <typename K, typename C, typename A>
    CStream& Serialize(std::set<K, C, A>& t, ObjectType&, LoadType&);
    template <typename K, typename C, typename A>
    CStream& Serialize(const std::set<K, C, A>& t, ObjectType&, std::size_t& serSize);
    template <typename K, typename C, typename A>
    CStream& Serialize(std::set<K, C, A>& t, ObjectType&, std::size_t& serSize);
...
public:
    ...
    void GetData(bytes& btData) const
    {
        btData.assign(gptr(), gptr() + size());
    }

    std::size_t TransferStream(CBufStream& ss, const std::size_t nSize)
    {
        std::size_t n = std::min(nSize, size());
        if (n > 0)
        {
            bytes btData;
            btData.resize(n);
            this->Read((char*)(btData.data()), n);
            ss.Write((char*)(btData.data()), n);
        }
        return n;
    }
...
protected:
    ...
    void GetBytes(bytes& out, std::size_t n);
    uint64 GetInt(const bytes& in);
...
{
    *this << CVarInt(t.size());
    for (typename std::set<K, C, A>::const_iterator it = t.begin(); it != t.end(); ++it)
    {
        *this << (*it);
    }
    return (*this);
...
template <typename K, typename C, typename A>
inline CStream& CStream::Serialize(std::set<K, C, A>& t, ObjectType&, SaveType&)
{
    *this << CVarInt(t.size());
    for (typename std::set<K, C, A>::iterator it = t.begin(); it != t.end(); ++it)
    {
        *this << (*it);
    }
    return (*this);
...
template <typename K, typename C, typename A>
inline CStream& CStream::Serialize(std::set<K, C, A>& t, ObjectType&, LoadType&)
{
    CVarInt var;
    *this >> var;
    for (uint64 i = 0; i < var.nValue; i++)
...
template <typename K, typename C, typename A>
inline CStream& CStream::Serialize(const std::set<K, C, A>& t, ObjectType&, std::size_t& serSize)
{
    CVarInt var(t.size());
    serSize += GetSerializeSize(var);
    for (typename std::set<K, C, A>::const_iterator it = t.begin(); it != t.end(); ++it)
...
template <typename K, typename C, typename A>
inline CStream& CStream::Serialize(std::set<K, C, A>& t, ObjectType&, std::size_t& serSize)
{
    CVarInt var(t.size());
    serSize += GetSerializeSize(var);
    for (typename std::set<K, C, A>::iterator it = t.begin(); it != t.end(); ++it)
