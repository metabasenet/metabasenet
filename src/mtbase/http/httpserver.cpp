...
std::string CHttpClient::GetPeerIp()
{
    boost::system::error_code ec;
    return pClient->GetRemote().address().to_string(ec);
}

uint16 CHttpClient::GetPeerPort()
{
    return pClient->GetRemote().port();
}
...
{
    CHttpUtil util;
    CHttpProfile profile(confHost.nLinkChainId, confHost.epHost);
                                   ...
                                   CBufStream& ssPayload)
{
    ...
    if (pHttpProfile == nullptr || pEventHttpReq == nullptr)
    {
    ...
    req.nPeerPort = pHttpClient->GetPeerPort();
