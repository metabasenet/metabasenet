            ...
            else
            {
                ...
                if (sched.GetCacheLocalPowBlock(hashBlock, block))
                {
                    ...
                    else
                    {
                        ...
                        {
                            StdLog("NetChannel", "SubmitCachePowBlock: add local pow block success, block: %s", hashBlock.GetHex().c_str());
                        }
                    }
            ...
            for (const network::CInv& inv : eventInv.data)
            {
                if (inv.nType == network::CInv::MSG_TX)
                {
                    ...
                    do
                    {
                        ...
                        {
                            StdTrace("NetChannel", "CEventPeerInv: peer: %s, received txinv: tx time error, tx time: %d, last time: %d, last height: %d, txid: %s, fork: %s",
                                     GetPeerAddressInfo(nNonce).c_str(), CTxId(inv.nHash).GetTxTime(),
                                     status.nBlockTime + MAX_TXINV_INTERVAL_TIME, status.nBlockHeight, inv.nHash.GetHex().c_str(), hashFork.GetHex().c_str());
                            break;
                        }
                ...
                else if (inv.nType == network::CInv::MSG_BLOCK)
                {
                    do
                    {
                        ...
                        {
                            StdTrace("NetChannel", "CEventPeerInv: peer: %s, block existed, height: %d, block hash: %s, fork: %s",
                                     GetPeerAddressInfo(nNonce).c_str(), nLocationHeight, inv.nHash.GetHex().c_str(), hashFork.GetHex().c_str());
...
bool CNetChannel::HandleEvent(network::CEventPeerGetData& eventGetData)
{
    ...
    for (const network::CInv& inv : eventGetData.data)
    {
        if (inv.nType == network::CInv::MSG_TX)
        {
            ...
            if (pTxPool->Get(inv.nHash, eventTx.data) || pBlockChain->GetTransaction(hashFork, uint256(), inv.nHash, eventTx.data))
            {
...
bool CNetChannel::HandleEvent(network::CEventPeerTx& eventTx)
{
    ...
    try
    {
        ...
        int nHeightAnchor;
        if (pBlockChain->GetBlockLocation(tx.hashFork, hashForkAnchor, nHeightAnchor)
        ...
        {
            StdLog("NetChannel", "CEventPeerTx: GetBlockLocation fail, txid: %s, hashFork: %s",
                   txid.GetHex().c_str(), tx.hashFork.GetHex().c_str());
                              ...
                              set<uint64>& setSchedPeer, set<uint64>& setMisbehavePeer, vector<pair<uint256, uint256>>& vRefNextBlock, bool fCheckPow)
{
    ...
    for (size_t i = 0; i < vBlockHash.size(); i++)
    {
        ...
        if (pBlock != nullptr)
        {
            ...
            if ((!pBlock->IsVacant() || !pBlock->txMint.to.IsNull()) && !sched.IsRepeatBlock(hashBlock))
            {
                if (!pBlockChain->VerifyRepeatBlock(hashFork, *pBlock, hashBlockRef))
                {
                    StdLog("NetChannel", "NetChannel Add New Block: block repeat mint, peer: %s, height: %d, block: %s",
                    ...
                    {
                        StdLog("NetChannel", "NetChannel Add New Block: Generate multiple repeat blocks, peer: %s, height: %d, block: %s",
                               GetPeerAddressInfo(nNonceSender).c_str(), CBlock::GetBlockHeightByHash(hashBlock), hashBlock.GetHex().c_str());
            ...
            if ((fCheckPow || hash != hashBlock) && pBlock->IsPrimary() && pBlock->IsProofOfWork())
            {
                ...
                {
                    StdLog("NetChannel", "NetChannel Add New Block: GetPowBlockState fail, peer: %s, height: %d, block: %s",
                           GetPeerAddressInfo(nNonceSender).c_str(), CBlock::GetBlockHeightByHash(hashBlock), hashBlock.GetHex().c_str());
                    setMisbehavePeer.insert(nNonceSender);
                    return;
                }
                if (!fVerifyPowBlock)
                {
                    ...
                    {
                        StdLog("NetChannel", "Add New Block VerifyPowBlock fail, peer: %s, height: %d, block: %s",
                               GetPeerAddressInfo(nNonceSender).c_str(), CBlock::GetBlockHeightByHash(hashBlock), hashBlock.GetHex().c_str());
                        setMisbehavePeer.insert(nNonceSender);
                        return;
                    }
                    ...
                    if (fLongChain)
                    {
                        ...
                        if (!pBlockChain->GetLastBlockStatus(pCoreProtocol->GetGenesisBlockHash(), status))
                        {
                            StdLog("NetChannel", "Add New Block GetLastBlock fail, peer: %s, height: %d, block: %s",
                                   GetPeerAddressInfo(nNonceSender).c_str(), CBlock::GetBlockHeightByHash(hashBlock), hashBlock.GetHex().c_str());
                            return;
                        }
                            ...
                            && hashFirstBlock == hashBlock)
                        {
                            ...
                            StdDebug("NetChannel", "Add New Block InnerBroadcastBlockInv: height: %d, block: %s",
                                     CBlock::GetBlockHeightByHash(hashBlock), hashBlock.GetHex().c_str());
                        }
                    }
                    else
                ...
                if (!fAddBlock)
                {
                    ...
                    StdDebug("NetChannel", "Add New Block cache pow block, peer: %s, height: %d, block: %s",
                             GetPeerAddressInfo(nNonceSender).c_str(), CBlock::GetBlockHeightByHash(hashBlock), hashBlock.GetHex().c_str());
                    continue;
                }
                ...
                if (!pBlockChain->GetBlockLocation(pBlock->hashPrev, hashForkPrev, nHeightPrev))
                {
                    ...
                    StdDebug("NetChannel", "Add New Block pow block not find prev, peer: %s, height: %d, block: %s",
                             GetPeerAddressInfo(nNonceSender).c_str(), CBlock::GetBlockHeightByHash(hashBlock), hashBlock.GetHex().c_str());
                    continue;
                }
            }
            ...
            if (err == OK)
            {
                StdDebug("NetChannel", "NetChannel Add New Block success, peer: %s, height: %d, block: %s",
                         GetPeerAddressInfo(nNonceSender).c_str(), CBlock::GetBlockHeightByHash(hashBlock), hashBlock.GetHex().c_str());
...

                {
                    ...
                    uint256 hashNextTxid = sched.GetNextTx(tx.to, 1);
                    if (hashNextTxid != 0)
                    {
                    ...
                    {
                        StdDebug("NetChannel", "NetChannel Add New Block: remove mint tx inv success, peer: %s, txid: %s",
                ...
                {
                    uint256 hashNextTxid = sched.GetNextTx(tx.from, tx.nTxNonce + 1);
                    if (hashNextTxid != 0)
                    {
                        set<uint64> setPrevSchedPeer;
                        set<uint64> setPrevMisbehavePeer;
                    ...
                    if (sched.RemoveInv(network::CInv(network::CInv::MSG_TX, txid), setSchedPeer))
                    {
                        StdDebug("NetChannel", "NetChannel Add New Block: remove tx inv success, peer: %s, txid: %s",
                                 GetPeerAddressInfo(nNonceSender).c_str(), txid.GetHex().c_str());
                    }
                }
            ...
            {
                StdLog("NetChannel", "NetChannel Add New Block: block already have, peer: %s, height: %d, block: %s",
                       GetPeerAddressInfo(nNonceSender).c_str(), CBlock::GetBlockHeightByHash(hashBlock), hashBlock.GetHex().c_str());
                set<uint64> setKnownPeer;
                sched.GetNextBlock(hashBlock, vBlockHash);
                sched.RemoveInv(network::CInv(network::CInv::MSG_BLOCK, hashBlock), setKnownPeer);
            ...
            {
                StdLog("NetChannel", "NetChannel Add New Block fail, peer: %s, block: %s, err: [%d] %s",
                           ...
                           set<uint64>& setSchedPeer, set<uint64>& setMisbehavePeer)
{
    ...
    for (size_t i = 0; i < vtx.size(); i++)
    {
        ...
        if (pTx != nullptr)
        {
            ...
            //    continue;
            //}
            if (!pTxPool->CheckTxNonce(hashFork, pTx->from, pTx->nTxNonce - 1))
            {
                uint64 nNextTxNonce = pTxPool->GetDestNextTxNonce(hashFork, pTx->from);
                StdDebug("NetChannel", "NetChannel Add New Tx: prev nonce error, nTxNonce: %ld, pool last nonce: %ld, , peer: %s, txid: %s",
                         pTx->nTxNonce, nNextTxNonce - 1, GetPeerAddressInfo(nNonceSender).c_str(), hashTx.GetHex().c_str());
                continue;
            }

            if (pTxPool->CheckTxNonce(hashFork, pTx->from, pTx->nTxNonce))
            {
                ...
                uint256 hashNextTxid = sched.GetNextTx(pTx->from, pTx->nTxNonce + 1);
                if (hashNextTxid != 0)
                {
