...
public:
    CBlockMakerProfile() {}
    CBlockMakerProfile(int nAlgoIn, const CDestination& dest, const uint256& nPrivKey, const uint32 nRewardRationIn)
      : nAlgo(nAlgoIn), destMint(dest), nRewardRation(nRewardRationIn)
    {
    ...
    bool GetTemplateData(bytes& btTemplateData) const
    {
        if (templMint == nullptr)
        {
            return false;
        }
        btTemplateData = templMint->GetTemplateData();
        return true;
    }
...
protected:
    ...
    bool ReplenishSubForkVacant(const uint256& hashFork, int nLastBlockHeight, uint256& hashLastBlock, const uint256& hashLastStateRoot, uint64& nLastBlockNumber,
