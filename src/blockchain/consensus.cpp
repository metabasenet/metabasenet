...
{
    templDelegate = CTemplate::CreateTemplatePtr(new CTemplateDelegate(keyDelegateIn.GetPubKey(), destOwner, nRewardRatioIn));
    destDelegate.SetTemplateId(templDelegate->GetTemplateId());
}

bool CDelegateContext::BuildEnrollTx(CTransaction& tx, const int nBlockHeight, const int64 nTime, const uint64 nTxNonce,
                                     const uint256& hashFork, const vector<unsigned char>& vchData)
{
    tx.SetNull();
    tx.nType = CTransaction::TX_CERT;
    tx.nTimeStamp = nTime;
    tx.hashFork = hashFork;
    tx.nTxNonce = nTxNonce;
    tx.from = destDelegate;
    tx.to = destDelegate;
    tx.nAmount = 1;
    ...
    tx.AddTxData(CTransaction::DF_CERTTXDATA, btTempData);

    uint256 hash = tx.GetSignatureHash();
    if (!keyDelegate.Sign(hash, tx.vchSig))
    {
...
bool CConsensus::HandleInitialize()
{
    ...
    if (MintConfig()->nPeerType != NODE_TYPE_FORK && !MintConfig()->destMpvss.IsNull() && MintConfig()->keyMpvss != 0)
    {
        ...
        CDelegateContext ctxt(key, MintConfig()->destMpvss, MintConfig()->nDelegateRewardRatio);
...
void CConsensus::PrimaryUpdate(const CBlockChainUpdate& update, CDelegateRoutine& routine)
{
    ...
    map<CDestination, uint64> mapEnrollTxNonce;
    ...
    for (int i = update.vBlockAddNew.size() - 1; i >= 0; i--)
    {
        ...
        else
        {
            ...
            uint256 nDelegateMinAmount = pBlockChain->GetDelegateMinEnrollAmount(hash);
            ...
            if (nDelegateMinAmount == 0 || !fGetVote)
            {
                ...
                {
                    StdError("CConsensus", "Primary Update: Get delegate min enroll amount fail, nDelegateMinAmount: %s, hash: %s",
                             CoinToTokenBigFloat(nDelegateMinAmount).c_str(), hash.GetHex().c_str());
                }
                if (!fGetVote)
                {
            ...
            else
            {
                     ...
                     it != result.mapEnrollData.end(); ++it)
                {
                    ...
                    StdTrace("CConsensus", "Primary Update: destDelegate: %s", destDelegate.ToString().c_str());
                    map<CDestination, CDelegateContext>::iterator mi = mapContext.find(destDelegate);
                    if (mi == mapContext.end())
                    {
                        StdTrace("CConsensus", "Primary Update: mapContext find fail, destDelegate: %s", destDelegate.ToString().c_str());
                        continue;
                    }
                    auto dt = mapDelegateVote.find(destDelegate);
                    if (dt == mapDelegateVote.end())
                    {
                        StdTrace("CConsensus", "Primary Update: mapDelegateVote find fail, destDelegate: %s", destDelegate.ToString().c_str());
                        continue;
                    }
                    if (dt->second < nDelegateMinAmount)
                    {
                        StdTrace("CConsensus", "Primary Update: not enough votes, vote: %s, weight ratio: %s, destDelegate: %s",
                                 CoinToTokenBigFloat(dt->second).c_str(), CoinToTokenBigFloat(nDelegateMinAmount).c_str(), destDelegate.ToString().c_str());
                        continue;
                    }

                    auto kt = mapEnrollTxNonce.find(destDelegate);
                    if (kt == mapEnrollTxNonce.end())
                    {
                        uint64 nTxNonce = pTxPool->GetCertTxNextNonce(destDelegate);
                        if (nTxNonce == 0)
                        {
                            StdError("CConsensus", "Primary Update: Get cert tx next nonce fail, destDelegate: %s", destDelegate.ToString().c_str());
                            continue;
                        }
                        kt = mapEnrollTxNonce.insert(make_pair(destDelegate, nTxNonce)).first;
                    }

                    CTransaction tx;
                    if (mi->second.BuildEnrollTx(tx, nBlockHeight, GetNetTime(), kt->second, pCoreProtocol->GetGenesisBlockHash(), it->second))
                    {
                        StdTrace("CConsensus", "Primary Update: Build enroll tx success, nonce: %ld, vote token: %s, weight ratio: %s, destDelegate: %s, block: [%d] %s",
                                 kt->second, CoinToTokenBigFloat(dt->second).c_str(), CoinToTokenBigFloat(nDelegateMinAmount).c_str(), (*it).first.ToString().c_str(),
                    ...
                    {
                        StdError("CConsensus", "Primary Update: Build enroll tx fail, vote token: %s, weight ratio: %s, destDelegate: %s, block: [%d] %s",
                                 CoinToTokenBigFloat(dt->second).c_str(), CoinToTokenBigFloat(nDelegateMinAmount).c_str(), (*it).first.ToString().c_str(),
                                 nBlockHeight, hash.GetHex().c_str());
                    }
                }
            }
