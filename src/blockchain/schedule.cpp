...
void CSchedule::RemoveInvState(const network::CInv& inv)
{
    ...
    if (inv.nType == network::CInv::MSG_TX)
    {
        ...
        CTransaction* ptx = GetTransaction(inv.nHash, nNonceSender);
        if (ptx)
        {
            auto it = mapRecvTxDest.find(ptx->GetFromAddress());
            if (it != mapRecvTxDest.end())
            {
                it->second.erase(ptx->GetNonce());
                if (it->second.empty())
                {
                    mapRecvTxDest.erase(it);
                }
            }
        }
    }
...
bool CSchedule::ReceiveTx(uint64 nPeerNonce, const uint256& txid, const CTransaction& tx, set<uint64>& setSchedPeer)
{
    ...
    if (it != mapState.end())
    {
        ...
        if (state.nAssigned == nPeerNonce && !state.IsReceived())
        {
            ...
            mapRecvTxDest[tx.GetFromAddress()][tx.GetNonce()] = txid;
            return true;
        }
    }
    return false;
...
uint256 CSchedule::GetNextTx(const CDestination& destFrom, const uint64 nNextTxNonce)
{
    auto it = mapRecvTxDest.find(destFrom);
    if (it != mapRecvTxDest.end())
    {
        auto mt = it->second.find(nNextTxNonce);
        if (mt != it->second.end())
        {
            return mt->second;
        }
...
bool CSchedule::CancelAssignedInv(uint64 nPeerNonce, const network::CInv& inv)
{
    ...
    if (it == mapState.end())
    {
        StdWarn("Schedule", "CancelAssignedInv: find inv fail, peer nonce: 0x%lx, inv: [%d] %s", nPeerNonce, inv.nType, inv.nHash.GetHex().c_str());
        return false;
    }
    CInvState& state = (*it).second;
    if (state.nAssigned != nPeerNonce)
    {
        StdWarn("Schedule", "CancelAssignedInv: state.nAssigned != nPeerNonce, peer nonce: 0x%lx, inv: [%d] %s", nPeerNonce, inv.nType, inv.nHash.GetHex().c_str());
        return false;
    }
    if (!state.IsReceived())
    {
        ...
        if (mt == mapPeer.end())
        {
            StdWarn("Schedule", "CancelAssignedInv: find peer fail, peer nonce: 0x%lx, inv: [%d] %s", nPeerNonce, inv.nType, inv.nHash.GetHex().c_str());
        }
        else
        {
        ...
        while (mt != setMissPrevTxInv.end())
        {
            ...
            if (it != mapState.end())
            {
                ...
                if (state.nAssigned == 0 && peer.KnownInvExists(inv))
                {
                    ...
                    {
                        StdLog("Schedule", "ScheduleKnownInv: inv timeout, peer nonce: 0x%lx, inv: [%d] %s, getcount: %d, waittime: %ld",
        ...
        for (const uint256& hash : listKnown)
        {
            ...
            if (it != mapState.end())
            {
                ...
                else if (state.IsReceived())
                {
                    ...
                    {
                        StdLog("Schedule", "ScheduleKnownInv: object timeout, peer nonce: 0x%lx, inv: [%d] %s, waittime: %ld",
