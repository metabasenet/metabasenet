...
void CStatP2pSynFork::AddTxSynStatData(const uint64 nTxCount, const bool fRecv)
{
    if (fRecv)
    {
        nStatSynRecvTxCount += nTxCount;
    }
    else
    {
        nStatSynSendTxCount += nTxCount;
    }
}
...
bool CDataStat::AddP2pSynTxSynStatData(const uint256& hashFork, const uint64 nTxCount, const bool fRecv)
{
    if (fStatWork)
    {
        boost::unique_lock<boost::mutex> lock(mutex);
        ...
        (*it).second.AddTxSynStatData(nTxCount, fRecv);
    }
    return true;
}
