...
public:
  ...
  std::string getCallerStr() {
    std::string Str;
    convertBytesToHexStr(Caller, Str, 64);
    return Str;
  }
  ...
  evmc::address getCallerEVMC() {
    evmc::address Addr;
    std::copy_n(Caller.cbegin(), 32, Addr.bytes);
    return Addr;
  }
  ...
  void setCaller(const std::string &Str) {
    convertHexStrToBytes(Str, Caller, 64);
  }
  ...
  std::string getAddressStr() {
    std::string Str;
    convertBytesToHexStr(Address, Str, 64);
    return Str;
  }
  ...
  evmc::address getAddressEVMC() {
    evmc::address Addr;
    std::copy_n(Address.cbegin(), 32, Addr.bytes);
    return Addr;
  }
  ...
  void setAddress(const std::string &Str) {
    convertHexStrToBytes(Str, Address, 64);
  }
...
private:
  ...
  /// Caller: 32 bytes sendor address.
  ...
  /// Address: 32 bytes destination address.
