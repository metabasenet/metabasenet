...
template <typename T>
class EEI : public Runtime::HostFunction<T>
{
public:
...
protected:
                                     ...
                                     uint32_t Off, uint32_t Bytes = 32)
    {
        ...
        evmc::uint256be Dst = {};
        if (auto Res = MemInst.getArray(Dst.bytes + (32 - Bytes), Off, Bytes, true))
        {
            return Dst;
        }
        else
    ...
    {
        return MemInst.setArray(Addr.bytes, Off, 32);
    }
                                  ...
                                  uint32_t CreateResOffset = 0)
    {
        ...
        if ((Msg.kind == evmc_call_kind::EVMC_CREATE
             || (Msg.kind == evmc_call_kind::EVMC_CALL && !evmc::is_zero(Msg.value)))
        ...
        /// Take additional gas.
        if ((Msg.kind == evmc_call_kind::EVMC_CALL || Msg.kind == evmc_call_kind::EVMC_CALLCODE)
        ...
        /// Return data.
        if ((Msg.kind == evmc_call_kind::EVMC_CREATE || Msg.kind == evmc_call_kind::EVMC_CREATE3)
            && CallRes.status_code == EVMC_SUCCESS)
        {
            if (auto Res = storeAddress(MemInst, CallRes.create_address, CreateResOffset); !Res)
            {
                return Unexpect(Res);
            }
            if (Msg.kind == evmc_call_kind::EVMC_CREATE3)
            {
                Env.getReturnData().assign(&(CallRes.create_address.bytes[0]), &(CallRes.create_address.bytes[0]) + sizeof(CallRes.create_address.bytes));
            }
            else
            {
        ...
        {
            Env.getReturnData().assign(CallRes.output_data, CallRes.output_data + CallRes.output_size);
        }
        else
        {
            Env.getReturnData().clear();
