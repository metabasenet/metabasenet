...
{
    return Code.size() >= 8 && Code[0] == 0 && Code[1] == 'a' && Code[2] == 's' && Code[3] == 'm';
}
        ...
        const struct evmc_message* msg, const uint8_t* code, size_t code_size)
{
    ...
    if (result.status_code == EVMC_SUCCESS && (*Store.getActiveModule())->getStartAddr())
    {
    ...
    if (isWasmBinary(ReturnData) && msg->kind == EVMC_CREATE && result.status_code != EVMC_REVERT)
    {
    ...
    //result.gas_left = (result.status_code == EVMC_FAILURE) ? 0 : msg->gas - usedGas;
