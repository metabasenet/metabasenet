...
//-----------------------------------------------------------------------
bool CBbPeerNet::HandleEvent(CEventPeerBlockSubscribe& eventSubscribe)
{
    CBufStream ssPayload;
    ssPayload << eventSubscribe;
    return SendChannelMessage(PROTO_CHN_BLOCK, eventSubscribe.nNonce, PROTO_CMD_BLOCK_SUBSCRIBE, ssPayload);
}

bool CBbPeerNet::HandleEvent(CEventPeerBlockUnsubscribe& eventUnsubscribe)
{
    CBufStream ssPayload;
    ssPayload << eventUnsubscribe;
    return SendChannelMessage(PROTO_CHN_BLOCK, eventUnsubscribe.nNonce, PROTO_CMD_BLOCK_UNSUBSCRIBE, ssPayload);
}

bool CBbPeerNet::HandleEvent(CEventPeerBlockBks& eventBks)
{
    CBufStream ssPayload;
    ssPayload << eventBks;
...
bool CBbPeerNet::HandleEvent(CEventPeerCerttxSubscribe& eventSubscribe)
{
    CBufStream ssPayload;
    ssPayload << eventSubscribe;
    return SendChannelMessage(PROTO_CHN_CERT_TX, eventSubscribe.nNonce, PROTO_CMD_CERTTX_SUBSCRIBE, ssPayload);
}

bool CBbPeerNet::HandleEvent(CEventPeerCerttxUnsubscribe& eventUnsubscribe)
{
    CBufStream ssPayload;
    ssPayload << eventUnsubscribe;
    return SendChannelMessage(PROTO_CHN_CERT_TX, eventUnsubscribe.nNonce, PROTO_CMD_CERTTX_UNSUBSCRIBE, ssPayload);
}

bool CBbPeerNet::HandleEvent(CEventPeerCerttxTxs& eventTxs)
{
    CBufStream ssPayload;
    ssPayload << eventTxs;
    return SendChannelMessage(PROTO_CHN_CERT_TX, eventTxs.nNonce, PROTO_CMD_CERTTX_TXS, ssPayload);
}

//-----------------------------------------------------------------------
bool CBbPeerNet::HandleEvent(CEventPeerUsertxSubscribe& eventSubscribe)
{
    CBufStream ssPayload;
    ssPayload << eventSubscribe;
    return SendChannelMessage(PROTO_CHN_USER_TX, eventSubscribe.nNonce, PROTO_CMD_USERTX_SUBSCRIBE, ssPayload);
}

bool CBbPeerNet::HandleEvent(CEventPeerUsertxUnsubscribe& eventUnsubscribe)
{
    CBufStream ssPayload;
    ssPayload << eventUnsubscribe;
    return SendChannelMessage(PROTO_CHN_USER_TX, eventUnsubscribe.nNonce, PROTO_CMD_USERTX_UNSUBSCRIBE, ssPayload);
}

bool CBbPeerNet::HandleEvent(CEventPeerUsertxTxs& eventTxs)
{
    CBufStream ssPayload;
    ssPayload << eventTxs;
...
void CBbPeerNet::DestroyPeer(CPeer* pPeer)
{
    ...
    if (pBbPeer->IsHandshaked())
    {
        ...
        if (pEventDeactive != nullptr)
        {
            ...
            if (pEventDeactiveBlock)
            {
                pBlockChannel->PostEvent(pEventDeactiveBlock);
            }
            if (pEventDeactiveCertTx)
            {
                pCertTxChannel->PostEvent(pEventDeactiveCertTx);
            }
...
bool CBbPeerNet::SendChannelMessage(int nChannel, uint64 nNonce, int nCommand, CBufStream& ssPayload)
{
...
bool CBbPeerNet::SendDelegatedMessage(uint64 nNonce, int nCommand, mtbase::CBufStream& ssPayload)
{
    CBbPeer* pBbPeer = static_cast<CBbPeer*>(GetPeer(nNonce));
    if (pBbPeer == nullptr)
    {
...
uint32 CBbPeerNet::BuildPing(mtbase::CPeer* pPeer, mtbase::CBufStream& ssPayload)
{
...
bool CBbPeerNet::HandlePeerHandshaked(CPeer* pPeer, uint32 nTimerId)
{
    ...
    if (setDNSeed.count(pBbPeer->GetRemote()) == 0)
    {
        ...
        if (pEventActiveBlock)
        {
            pBlockChannel->PostEvent(pEventActiveBlock);
        }
        if (pEventActiveCertTx)
        {
            pCertTxChannel->PostEvent(pEventActiveCertTx);
        }
...
bool CBbPeerNet::HandlePeerRecvMessage(CPeer* pPeer, int nChannel, int nCommand, CBufStream& ssPayload)
{
    ...
    else if (nChannel == PROTO_CHN_DATA)
    {
        ...
        case PROTO_CMD_TX:
        {
            ...
            {
                ssPayload >> pEvent->data;
                CInv inv(CInv::MSG_TX, pEvent->data.hash);
        ...
        case PROTO_CMD_BLOCK:
        {
            ...
            {
                ssPayload >> pEvent->data;
                CInv inv(CInv::MSG_BLOCK, pEvent->data.hash);
                CancelTimer(pBbPeer->Responded(inv));
                pNetChannel->PostEvent(pEvent);
                return true;
            }
    ...
    else if (nChannel == PROTO_CHN_BLOCK)
    {
        uint256 hashFork;
        ssPayload >> hashFork;
        switch (nCommand)
        {
        case PROTO_CMD_BLOCK_SUBSCRIBE:
        {
            CEventPeerBlockSubscribe* pEvent = new CEventPeerBlockSubscribe(pBbPeer->GetNonce(), hashFork);
            if (pEvent != nullptr)
            {
                ssPayload >> pEvent->data;
        ...
        break;
        case PROTO_CMD_BLOCK_UNSUBSCRIBE:
        {
            CEventPeerBlockUnsubscribe* pEvent = new CEventPeerBlockUnsubscribe(pBbPeer->GetNonce(), hashFork);
            if (pEvent != nullptr)
            {
                ssPayload >> pEvent->data;
                pBlockChannel->PostEvent(pEvent);
        ...
        break;
        case PROTO_CMD_BLOCK_BKS:
        {
            CEventPeerBlockBks* pEvent = new CEventPeerBlockBks(pBbPeer->GetNonce(), hashFork);
            if (pEvent != nullptr)
            {
                ssPayload >> pEvent->data;
                pBlockChannel->PostEvent(pEvent);
    ...
    else if (nChannel == PROTO_CHN_CERT_TX)
    {
        ...
        switch (nCommand)
        {
        case PROTO_CMD_CERTTX_SUBSCRIBE:
        {
            CEventPeerCerttxSubscribe* pEvent = new CEventPeerCerttxSubscribe(pBbPeer->GetNonce(), hashFork);
            if (pEvent != nullptr)
            {
                ssPayload >> pEvent->data;
                pCertTxChannel->PostEvent(pEvent);
                return true;
            }
        }
        break;
        case PROTO_CMD_CERTTX_UNSUBSCRIBE:
        {
            CEventPeerCerttxUnsubscribe* pEvent = new CEventPeerCerttxUnsubscribe(pBbPeer->GetNonce(), hashFork);
            if (pEvent != nullptr)
            {
                ssPayload >> pEvent->data;
                pCertTxChannel->PostEvent(pEvent);
        ...
        break;
        case PROTO_CMD_CERTTX_TXS:
        {
            CEventPeerCerttxTxs* pEvent = new CEventPeerCerttxTxs(pBbPeer->GetNonce(), hashFork);
            if (pEvent != nullptr)
            {
                ssPayload >> pEvent->data;
                pCertTxChannel->PostEvent(pEvent);
    ...
    else if (nChannel == PROTO_CHN_USER_TX)
    {
        uint256 hashFork;
        ssPayload >> hashFork;
        switch (nCommand)
        {
        case PROTO_CMD_USERTX_SUBSCRIBE:
        {
            CEventPeerUsertxSubscribe* pEvent = new CEventPeerUsertxSubscribe(pBbPeer->GetNonce(), hashFork);
            if (pEvent != nullptr)
            {
                ssPayload >> pEvent->data;
                pUserTxChannel->PostEvent(pEvent);
                return true;
            }
        }
        break;
        case PROTO_CMD_USERTX_UNSUBSCRIBE:
        {
            CEventPeerUsertxUnsubscribe* pEvent = new CEventPeerUsertxUnsubscribe(pBbPeer->GetNonce(), hashFork);
            if (pEvent != nullptr)
            {
                ssPayload >> pEvent->data;
                pUserTxChannel->PostEvent(pEvent);
        ...
        break;
        case PROTO_CMD_USERTX_TXS:
        {
            CEventPeerUsertxTxs* pEvent = new CEventPeerUsertxTxs(pBbPeer->GetNonce(), hashFork);
            if (pEvent != nullptr)
            {
                ssPayload >> pEvent->data;
                pUserTxChannel->PostEvent(pEvent);
...
uint32 CBbPeerNet::CreateSeq(uint64 nNonce)
{
    ...
    return metabasenet::crypto::crc24q((const unsigned char*)ss.GetData(), ss.GetSize());
}
