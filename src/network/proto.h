...
enum
{
    ...
    PROTO_CHN_CERT_TX = 4,
    PROTO_CHN_USER_TX = 5,
    PROTO_CHN_BLOCK_VOTE = 6,
    PROTO_CHN_BLOCK_CROSS_PROVE = 7
};
...
enum
{
    PROTO_CMD_BLOCK_SUBSCRIBE = 1,
    PROTO_CMD_BLOCK_UNSUBSCRIBE = 2,
    PROTO_CMD_BLOCK_BKS = 3,
    PROTO_CMD_BLOCK_NEXT_PREVBLOCK = 4,
    PROTO_CMD_BLOCK_PREV_BLOCKS = 5,
    PROTO_CMD_BLOCK_GET_BLOCK_REQ = 6,
    PROTO_CMD_BLOCK_GET_BLOCK_RSP = 7,
};

enum
{
    PROTO_CMD_CERTTX_SUBSCRIBE = 1,
    PROTO_CMD_CERTTX_UNSUBSCRIBE = 2,
    PROTO_CMD_CERTTX_TXS = 3,
};

enum
{
    PROTO_CMD_USERTX_SUBSCRIBE = 1,
    PROTO_CMD_USERTX_UNSUBSCRIBE = 2,
    PROTO_CMD_USERTX_TXS = 3,
};
...
enum
{
    PROTO_CMD_BLOCKVOTE_DATA = 1,
};

enum
{
    PROTO_CMD_BLOCK_CROSS_PROVE_DATA = 1,
};
...
public:
    ...
    uint32 GetHeaderChecksum() const
    {
        ...
        *(uint32*)&buf[9] = nPayloadChecksum;
        return metabasenet::crypto::crc24q(buf, 13);
    }
    bool Verify() const
    {
    ...
    {
        return ((nChannel << 4) | (nCommand & 0xF));
    }

protected:
    void Serialize(mtbase::CStream& s, mtbase::SaveType&)
    {
    ...
    void Serialize(mtbase::CStream& s, std::size_t& serSize)
    {
        (void)s;
...
class CEndpoint : public mtbase::CBinary
{
public:
    enum
    {
